...
### 流程
1. **连续黑度**：每帧计算 `darkness[i] = max(gray) - gray[i]`，不再依赖固定阈值 `2000`
2. **传感器权重**：从最左到最右依次为 `-7, -5, -3, -1, +1, +3, +5, +7`
3. **误差**：`error = Σ(darkness_i × weight_i) / Σ(darkness_i)`
   - 黑线居中时误差约为 0；偏左为负，偏右为正
   - 灰度最大最小差小于 `120` 时判定为无有效黑白对比，立即停车并清除 PID 历史状态
4. **位置式 PID 输出**：`pid = KP×error + KI×∫error×dt + KD×d(error)/dt`，其中 `dt=0.010s`
5. **差速**：`M1 = base - pid`，`M2 = base + pid`

### 默认参数

```c
LF_DEFAULT_KP  = 60      // 比例系数（权重幅值扩大后等比例缩小）
LF_DEFAULT_KI  = 0       // 积分系数（未启用）
LF_DEFAULT_KD  = 0       // 微分系数（未启用）
LF_DEFAULT_SPD = 500     // 基础速度 (0~1000)
LF_MIN_CONTRAST = 120    // 最小有效黑白灰度差
...
| 2026-07-28 | 手动左右转及循迹修正方向相反 | 交换左右转轮速方向；循迹采用 `-7~-1,+1~+7` 权重及对应差速方向；PID 改为带 10ms `dt` 的位置式计算并抑制首次微分突跳 |
...
直线段使用 MPU9250 Z 轴角速度积分保持航向，圆弧段使用八路灰度传感器和位置式 PID。程序通过黑色圆弧的出现和消失判断 B、C、D、A，不依赖电机编码器。每到达一个点，板载 `PB22` LED 和 `PB27` 蜂鸣器提示一次。

MPU9250 使用软件 I2C：`PA0=SDA`、`PA1=SCL`、地址 `0x68`。开始任务后必须保持小车静止约 1 秒，等待陀螺仪零偏校准完成。
